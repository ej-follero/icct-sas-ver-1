...
import { Megaphone, FileText, Tag, Calendar, Save, X, AlertCircle } from "lucide-react";
...
export function EditAnnouncementDialog({ open, onOpenChange, announcement, onAnnouncementUpdated }: EditAnnouncementDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ submit: error instanceof Error ? error.message : "Failed to update announcement" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden bg-white/95 backdrop-blur-sm border border-blue-200 shadow-2xl rounded-2xl p-0 mx-2 my-1 sm:mx-4 sm:my-1 md:mx-6 md:my-1 lg:mx-8 lg:my-1 flex flex-col h-full">
        ...
        <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-blue-800 rounded-t-2xl p-6 relative flex-shrink-0">
          <div className="flex items-start gap-4 pr-24">
            ...
            <div className="flex-1">
              ...
              <p className="text-blue-100 text-sm mt-1 font-medium">Update announcement information</p>
            </div>
          </div>
        ...
        <DialogFooter className="!flex !justify-end gap-3 p-6 flex-shrink-0 border-t border-gray-200 bg-white">
          <Button 
            variant="outline" 
            onClick={() => onOpenChange(false)} 
            disabled={isLoading}
