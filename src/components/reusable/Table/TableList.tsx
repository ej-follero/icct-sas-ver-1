          ...
          <TableRow>
            {columns.map((col, i) => (
              <TableHead
                ...
                className={`bg-blue-50 text-blue-900 font-semibold text-sm border-b border-blue-100 ${col.headerClassName || col.className || ''}`}
              >
                {col.sortable && onSort ? (
        ...
        <TableBody>
          {loading ? (
            ...
            data.map((item, rowIndex) => {
              ...
              const safeItemId = itemId !== undefined && itemId !== null && itemId !== '' ? itemId : `row-${rowIndex}`;
              const isExpanded = expandedRowIds.includes(itemId);
              return (
              <React.Fragment key={`row-fragment-${safeItemId}`}>
              <TableRow key={`row-${safeItemId}`}>
                {columns.map((col, colIdx) => {
                  const cellKey = `cell-${safeItemId}-${String(col.accessor)}-${colIdx}`;
              ...
              {isExpanded &&
                (() => {
                  ...
                  if (expanderCol && expanderCol.expandedContent) {
                    return (
                      <TableRow key={`expanded-row-${safeItemId}`}>
